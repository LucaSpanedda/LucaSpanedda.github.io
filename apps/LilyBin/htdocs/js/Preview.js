define(['jquery', 'plugins/spinner'], function($) {
      ...
      function(event) {
        ...
        switch (event.data.type) {
          ...
          case 'log':
            ...
            _this.error[event.data.visible ? 'show' : 'hide'](
              'Compile Log',
              ''
            );
            break;
  ...
  Preview.prototype.setPdfSrc = function() {
    var msg = {
      id: this.id,
      url:
        'https://s3-us-west-2.amazonaws.com/lilybin-scores/' +
        this.id +
        '.pdf' +
        this.cacheBuster,
      midi: this.files.midi
    };
    this.iframe[0].contentWindow.postMessage(msg, document.location.origin);
  };
  ...
  Preview.prototype.resize = function() {
    this.iframe.css({
      height:
        this.container.height() - this.error.$parent[0].offsetHeight + 'px'
    });
  };
  return Preview;
});
